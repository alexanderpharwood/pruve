...
import validateArrayOf from "./validators/arrayOf.js";
...
import validateContains from "./validators/contains.js";
import validateFunction from "./validators/function.js";
import validateUndefined from "./validators/undefined.js";
import validateTelephone from "./validators/telephone.js";
...
export default class {
  ...
  validate() {
    if (
      validateObject(this.values) === false &&
      validateArray(this.values) === false
    ) {
      throw new TypeError(
        "Values must be an object or array, with properties to validate."
      );
    }

    if (validateObject(this.rules) === false) {
      throw new TypeError(
        "Rules must be an object, with properties to validate."
      );
    }

    for (const prop in this.rules) {
      const ruleset = validateString(this.rules[prop])
        ? this.rules[prop].split(".")
        : this.rules[prop];
      ...
      if (validateObject(rule) === true) {
        for (const testKey in rule) {
          ...
          const potentialError =
            this.messageInContext(key, testKey) ||
      ...
      switch (ruleProps.rule) {
        case "string":
          if (validateString(value) === false) {
            const message =
              this.messageInContext(key, ruleProps.rule) ||
        ...
        case "undefined":
          ...
          break;
        case "telephone":
          if (validateTelephone(value) === false) {
            const message =
              this.messageInContext(key, ruleProps.rule) ||
              ErrorFactory.telephoneValidationError();
        ...
        case "url":
          if (validateUrl(value) === false) {
            const message =
              this.messageInContext(key, ruleProps.rule) ||
              ErrorFactory.urlValidationError();
        ...
        case "between":
          if (
            validateBetween(
              value,
              parseInt(ruleProps.conditions[0]),
              parseInt(ruleProps.conditions[1])
            ) === false
          ) {
            const message =
              this.messageInContext(key, ruleProps.rule) ||
              ErrorFactory.betweenValidationError(
                ruleProps.conditions[0],
                ruleProps.conditions[1]
              );
        ...
        case "has":
          ...
          break;
        case "contains":
          if (validateContains(value, ruleProps.conditions) === false) {
            const message =
              this.messageInContext(key, ruleProps.rule) ||
              ErrorFactory.containsValidationError(ruleProps.conditions);
        ...
        case "eachHas":
          ...
          break;
        case "arrayOf":
          if (validateArrayOf(value, ruleProps.conditions) === false) {
            const message =
              this.messageInContext(key, ruleProps.rule) ||
              ErrorFactory.arrayOfValidationError(ruleProps.conditions);
